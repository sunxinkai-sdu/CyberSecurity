...
*计算C1 = k·G  计算S = [h]Pb  计算k·Pb = (x2,y2)

* 使用KDF生成密钥t

* 计算C2 = M ⊕ t  计算C3 = Hash(x2||M||y2)

##### 解密流程：

* 从C1提取点坐标

* 计算S = [h]C1  计算d·C1 = (x2,y2)
...
### 伪造中本聪的数字签名
实际上中本聪的私钥是未知的，但是可以模拟一种情况：如果中本聪在多个交易中重复使用了相同的随机数k，那么我们就可以从中恢复出他的私钥。
#### 实现思路
##### 获取两个真实签名：

从区块链历史中获取中本聪的两个交易签名

这两个签名必须使用相同的随机数k（可通过r值相同判断）
##### 恢复k
$k= \frac {e_{1}-e_{2}}{s_{1}-s_{2}}$
##### 恢复私钥d
$d= \frac {s_{1}k-e_{1}}{r}$
##### 伪造新签名
使用恢复的私钥和随机数k生成新签名
### 实验结果
详细请见project5-c.png
