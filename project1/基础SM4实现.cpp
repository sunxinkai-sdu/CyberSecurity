...
void sm4_encrypt_basic(const uint8_t in[16], uint8_t out[16], const uint32_t rk[32]) {
    ...
    for (int round = 0; round < 32; round++) {
        ...
        // 应用线性变换
        tmp = linear_transform(tmp);

        // 更新状态
        state[round + 4] = state[round] ^ tmp;
    }
...
// CBC模式加密（基础版本）
void cbc_encrypt_basic(const vector<uint8_t>& plaintext, vector<uint8_t>& ciphertext,
    ...
    for (size_t i = 0; i < block_count; i++) {
        ...
        // SM4加密（基础版本）
        sm4_encrypt_basic(block, &ciphertext[i * 16], rk);
...
int main() {
    ...
    // CBC模式加密（基础版本）
    cbc_encrypt_basic(padded_plaintext, ciphertext, rk, iv);
    ...
    cout << "\nBase SM4 encryption time: " << fixed << setprecision(3) << elapsed << " ms\n";
