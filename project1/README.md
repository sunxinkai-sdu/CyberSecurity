...
### 利用T-table优化SM4的原理
#### 优化原理
T-table优化的核心思想是通过预计算来减少加密过程中的计算量。SM4的轮函数包含两个主要操作：非线性变换（S盒替换）、线性变换L（多个循环移位和异或）
...
### 利用GFNI优化SM4的原理
#### 优化原理
GFNI（Galois Field New Instructions）是Intel推出的专用指令集，可直接在硬件层面执行伽罗瓦域运算。SM4的S盒可分解为：有限域GF(2⁸)上的逆运算、仿射变换
...
具体结果参考图片
### 性能对比
优化技术    周期/字节 (1KB)   加速比   并行度  硬件要求
基础实现	  28.5 cycles	      1.0x	     1	   任何x86
T-table	    7.2 cycles	      4.0x	     1	   任何x86
AES-NI	    2.1 cycles	      13.6x	     4	   SSE4.1+
GFNI+AVX512	0.6 cycles	      47.5x	    16	   AVX512+

## SM4-GCM工作模式的软件优化实现
### SM4-GCM工作模式介绍
GCM（伽罗瓦/计数器模式）：结合了CTR模式和身份验证的加密模式，提供了加密和认证功能。它适合对数据进行加密时也需要验证数据完整性的场景，安全性极高，但复杂度较高，处理时间相对较长。

机密性保障：CTR模式提供流加密、每次加密使用唯一计数器值、密钥流不可预测

完整性保障：认证标签绑定：IV、密文、附加数据、任何篡改导致GHASH结果变化、认证密钥H由主密钥派生
### 关键优化说明
#### 加密函数优化
* 使用寄存器变量（r0-r3）替代数组访问
...
* 使用_byteswap_ulong加速字节序转换

* 优化边界处理，减少条件判断
#### GHASH优化
* 直接使用指针操作数据，避免vector插入开销
...
具体结果参考图片project1-b-基础版本和project1-b
